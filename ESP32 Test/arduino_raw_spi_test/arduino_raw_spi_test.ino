...
void setup() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Serial.printf(
        "  %s: pulldown=%d pullup=%d driveHI=%d driveLO=%d%s\n",
        spi_names[i],
        pd,
        pu,
        hi,
  ...
  digitalWrite(PIN_PWR, HIGH);  // enable HAT boost converter
  ...
  // Note: 0x06 (Booster Soft Start) intentionally omitted.
  ...
  bb_cmd(0x10);
  bb_buf(0x00, EPD_W * EPD_H / 8);
  Serial.println("  done.");

  Serial.println("DTM2 (0xFF = black, new frame)...");
  bb_cmd(0x13);
  bb_buf(0xFF, EPD_W * EPD_H / 8);
  Serial.println("  done.");
