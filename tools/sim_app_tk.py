...
def load_model(repo_root):
    ...
    # Kitchen-style tasks. If absent, seed demo content.
    tasks = d.get("tasks")
    reminders = []
    if isinstance(tasks, list) and tasks:
        ...
        # Fallback: map old reminders to shopping, and add a few fridge items for the new UI demo.
...
class Simulator(tk.Tk):
    def __init__(self):
        ...
        self.status.grid(row=1, column=0, sticky="ew", padx=10, pady=(0, 4))
        ...
        self.help.grid(row=2, column=0, sticky="ew", padx=10, pady=(0, 10))
        ...
        # Periodic tick for idle + delayed reorder
    ...
    def _render(self):
        ...
        img = Image.new("RGB", (w, h), bg if isinstance(bg, tuple) else (229, 229, 229))
        render_app(img, self.state, self.fonts, self.theme)
        self._photo = ImageTk.PhotoImage(img)
        self.preview.configure(image=self._photo)

        ui = self.state.ui
        self.status.configure(
            text=f"screen={ui.screen.value} focus={ui.focused_index} page={ui.page} idle={ui.idle} pending_reorder={ui.pending_reorder}"
        )
