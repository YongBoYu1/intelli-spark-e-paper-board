...
def _theme(theme: dict) -> dict:
    ...
    t.setdefault("bp_weather_right_inset", 18)
...
def render_home_kitchen_portrait(image, state: AppState, fonts, theme: dict) -> None:
    ...
    inner_h = max(1, y1 - y0)
    header_h = max(160, int(inner_h * float(t["bp_header_ratio"])))
    ...
    # but keep enough headroom to avoid clipping ascenders.
    time_y = max(-4, hy0 + int(t.get("bp_time_nudge_y", -4)))
